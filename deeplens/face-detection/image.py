...
class ImageUploader(Thread):
    ...
    def upload(self, img, index=0, metadata={}, timestamp=time.time()):
        try:
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), 90]
            success, jpg_data = cv2.imencode('.jpg', img, encode_param)
            if success:
                now = datetime.datetime.now()
                key = "faces/{:02d}_{:02d}/{:02d}_{:02d}/{}_{}.jpg".format(
                    now.month, now.day,now.hour, now.minute,int(timestamp),index)
                item = {
